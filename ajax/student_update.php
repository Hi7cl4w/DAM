...
if (version_compare(PHP_VERSION, '5.3.7', '<')) {
   ...
   require_once("libraries/password_compatibility_library.php");
    ...
    if ($_POST['password_new'] !== $_POST['confirm_password']) {
    ...
    } elseif (empty($_POST['fname']) && empty($_POST['lname']) && empty($_POST['adyear']) && empty($_POST['email'])&& empty($_POST['password_new'])&& empty($_POST['confirm_password'])) {
        ...
        if ($_POST['password_new'] != "") {
            ...
            $sql2 = "UPDATE users SET " . $pass . " WHERE prn='" . $prn . "';";
            $query = mysql_query($sql2);
            echo "<br>Password has been updated</br>";
        }
        if ($_POST['fname'] != "") {
            $fname = "fname='" . $_POST['fname'] . "'";
            $sql2 = "UPDATE student_tbl SET " . $fname . " WHERE prn='" . $prn . "';";
            $query = mysql_query($sql2);
       ...
       /* if ($_POST['user_status'] != "") {
        ...
        if ($_POST['lname'] != "") {
            ...
            $sql = "UPDATE student_tbl SET " . $lname . " WHERE prn='" . $prn . "';";
            $query = mysql_query($sql);
            echo "<br>last name has been updated</br>";
        }
        if ($_POST['adyear'] != "") {
            $adyear = "admission_year='" . $_POST['adyear'] . "'";
            $sql = "UPDATE student_tbl SET " . $adyear . " WHERE prn='" . $prn . "';";
            $query = mysql_query($sql);
            echo "<br>Admission year has been updated</br>";
        }
        if ($_POST['email'] != "") {
            $email = "email='" . $_POST['email'] . "'";
            $sql = "UPDATE student_tbl SET " . $email . " WHERE prn='" . $prn . "';";
            $query = mysql_query($sql);
