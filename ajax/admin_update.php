<?php
   ...
   require_once("libraries/password_compatibility_library.php");
    ...
    if ($_POST['password_new'] !== $_POST['confirm_password']) {
    ...
    } elseif (empty($_POST['fname']) && empty($_POST['lname']) && empty($_POST['des']) && empty($_POST['email'])&& empty($_POST['password_new'])&& empty($_POST['confirm_password'])) {
        ...
        if ($_POST['fname'] != "") {
            ...
            $sql = "UPDATE admin_tbl SET " . $fname . " WHERE prn='" . $prn . "';";
            $query = mysql_query($sql);
       ...
       /* if ($_POST['user_status'] != "") {
        ...
        if ($_POST['lname'] != "") {
            ...
            $sql = "UPDATE admin_tbl SET " . $lname . " WHERE prn='" . $prn . "';";
            $query = mysql_query($sql);
            echo "<br>last name has been updated</br>";
        }
        if ($_POST['des'] != "") {
            $des = "designation='" . $_POST['des'] . "'";
            $sql = "UPDATE admin_tbl SET " . $des . " WHERE prn='" . $prn . "';";
            $query = mysql_query($sql);
            echo "<br>designation has been updated</br>";
        }
        if ($_POST['email'] != "") {
            $email = "email='" . $_POST['email'] . "'";
            $sql = "UPDATE admin_tbl SET " . $email . " WHERE prn='" . $prn . "';";
            $query = mysql_query($sql);
